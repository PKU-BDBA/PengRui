...
def get_adj(data_path, generated_data_path):
    ...
    if 1:
        workbook = openpyxl.load_workbook(data_path)  #openpyxl用来处理excel表格
        ...
        for sheet_name in ['Batch 1', 'Batch 2', 'Batch 3']:
            ...
            sheet_X = sheet_X[1:]  #把细胞的横坐标取出来
            sheet_Y = [item.value for item in list(worksheet.columns)[2]]
            sheet_Y = sheet_Y[1:]  #把细胞的纵坐标取出来
            sheet_X = np.array(sheet_X).astype(np.float)
            sheet_Y = np.array(sheet_Y).astype(np.float) #创建numpy数组
            ...
            num_cells += sheet_num_cells   #总的细胞数量
    ...
    for threshold in [200]:
        num_big = np.where(all_distance_array<threshold)[0].shape[0]  # 距离小于200的数量
...
def get_attribute(data_path, generated_data_path):
    ...
    all_data = np.loadtxt(data_path, str, delimiter = ",")  #加载数据
    all_features = all_data[1:, 1: ]    #将gene名称列和细胞名称列给去除
    all_features[all_features == ''] = 0.0  #将空值给赋为0
    all_features = all_features.astype(np.float) #将数据形式变为np.float
    all_features = np.swapaxes(all_features, 0, 1) #转置
    print('feature shape: ', all_features.shape)
    np.save( generated_data_path + 'features.npy', all_features) #保存

    all_features_batch_1, all_features_batch_2, all_features_batch_3 = [], [], []

    #根据类别拆分数据
    all_cell_names = all_data[0, 1:] #细胞名称
    all_batch_info = []
